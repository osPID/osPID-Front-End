  ...
  try
  {
    ...
    if (reader!=null)
    {
      for (int i=0; i<prefVals.length; i++)
      {
        ...
        prefVals[i] = PApplet.parseFloat(reader.readLine());
        println(prefVals[i]);
      }
    }
  }
...
public void Save_Preferences()
{
  ...
  for (int i=0; i<prefs.length; i++)
  {
    try
    {
      prefVals[i] = PApplet.parseFloat(controlP5.getController(prefs[i]).getValueLabel().getText());
    }
    catch(Exception ex) {
    }
  }
...
public void Connect()
{
  ...
  if(!madeContact)
  {
    try
    {
      ...
      int selected=PApplet.parseInt(r1.getValue());
      println(selected);
...
public void ProfileRunTime()
{
  ...
  if (lastReceiptTime+3000<millis())
  {
    for (int i=0; i<6; i++)
    { 
      ((controlP5.Textlabel)controlP5.getController("profstat"+i)).setValue("");
      ((controlP5.Textlabel)controlP5.getController("dashstat"+i)).setValue("");
      curProfStep=-1;
...
public void populateProfileTab()
{
  ...
  if (profs.length>0) LBPref.setValue(0.0f);
...
public void addToRadioButton(RadioButton theRadioButton, String theName, int theValue ) 
{
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "osPID_FrontEnd" };
    if (passedArgs != null) {
